...
#define CON_STATUS_1_LED	DK_LED2
#define CON_STATUS_2_LED	DK_LED3
#define NFC_LED			DK_LED4
#define KEY_TEXT_MASK		DK_BTN1_MSK
#define KEY_SHIFT_MASK		DK_BTN2_MSK
#define KEY_DISCONNECT_MASK	DK_BTN3_MSK
...
static struct conn_mode {
	struct bt_conn *conn;
	bool in_boot_mode;
	uint32_t conn_inst; // Always incrementing connection instance, highest value is the latest connection (assumed to never wrap around)
...
static void update_connected_leds(void)
{
	size_t conn_count = 0;
	...
	/* Update connection status LEDs */
	if (conn_count >= 1) {
		dk_set_led_on(CON_STATUS_1_LED);
	} else {
		dk_set_led_off(CON_STATUS_1_LED);
	}

	if (conn_count >= 2) {
		dk_set_led_on(CON_STATUS_2_LED);
	} else {
		dk_set_led_off(CON_STATUS_2_LED);
	}
}
...
static void connected(struct bt_conn *conn, uint8_t err)
{
	...
	uint32_t conn_inst_max = 0;
	for (size_t i = 0; i < CONFIG_BT_HIDS_MAX_CLIENT_COUNT; i++) {
		if (conn_mode[i].conn_inst > conn_inst_max) {
			conn_inst_max = conn_mode[i].conn_inst;
		}
	}
	...
	for (size_t i = 0; i < CONFIG_BT_HIDS_MAX_CLIENT_COUNT; i++) {
		if (!conn_mode[i].conn) {
			...
			conn_mode[i].conn_inst = conn_inst_max + 1;
			break;
		}
	}

	update_connected_leds();
...
static void disconnect_oldest_conn(void)
{
	int err;
	char addr[BT_ADDR_LE_STR_LEN];
	struct bt_conn *conn = NULL;
	uint32_t conn_inst_min = UINT32_MAX;

	for (size_t i = 0; i < CONFIG_BT_HIDS_MAX_CLIENT_COUNT; i++) {
		if (conn_mode[i].conn && conn_mode[i].conn_inst < conn_inst_min) {
			conn_inst_min = conn_mode[i].conn_inst;
			conn = conn_mode[i].conn;
		}
	}

	bt_addr_le_to_str(bt_conn_get_dst(conn), addr, sizeof(addr));
	printk("Disconnecting oldest connection (%s)\n", addr);
...
static void button_changed(uint32_t button_state, uint32_t has_changed)
{
	...
	if ((has_changed & KEY_DISCONNECT_MASK) && (button_state & KEY_DISCONNECT_MASK)) {
		disconnect_oldest_conn();
		return;
	}
